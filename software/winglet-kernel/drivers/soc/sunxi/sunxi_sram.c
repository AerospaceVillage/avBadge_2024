...
#define SUNXI_SRAM_REMAP_REG		0x08
#define SUNXI_SRAM_REMAP_ENABLE_BIT	0

int sunxi_sram_remap_set(int map_local)
{
	u32 val;
	...
	val = readl(base + SUNXI_SRAM_REMAP_REG);
	val &= ~(1 << SUNXI_SRAM_REMAP_ENABLE_BIT);
	if (map_local) {
		val |= SUNXI_SRAM_REMAP_LOCAL << SUNXI_SRAM_REMAP_ENABLE_BIT;
	}
	else {
		val |= SUNXI_SRAM_REMAP_DSP << SUNXI_SRAM_REMAP_ENABLE_BIT;
	}
	writel(val, base + SUNXI_SRAM_REMAP_REG);

	spin_unlock(&sram_lock);

	return 0;
}
EXPORT_SYMBOL(sunxi_sram_remap_set);
...
int sunxi_sram_remap_get(void) {
	...
	return val & (1 << SUNXI_SRAM_REMAP_ENABLE_BIT);
}
EXPORT_SYMBOL(sunxi_sram_remap_get);
					     ...
					     unsigned int reg)
{
	...
	if (reg == SUNXI_SYS_LDO_CTRL_REG && variant->has_ldo_ctrl)
		return true;
	if (reg == SUNXI_SRAM_REMAP_REG && variant->has_dsp)
		return true;
