...
struct sun4i_usb_phy_data {
	...
	struct platform_device *axp2585_pdev;
...
static int sun4i_usb_phy0_get_axp2585_id_det(struct sun4i_usb_phy_data *data)
{
	enum axp2585_usb_id_mode mode;
	int ret;

	ret = axp2585_batt_get_id_mode(data->axp2585_pdev, &mode);
	if (ret) {
		pr_warn("%s: Could not read axp2585 id mode: %d, falling back to peripheral\n",
			__func__, ret);
		return 1;
	}
...
static int sun4i_usb_phy0_get_id_det(struct sun4i_usb_phy_data *data)
{
	...
	case USB_DR_MODE_OTG:
		...
		else if (data->axp2585_pdev)
			return sun4i_usb_phy0_get_axp2585_id_det(data);
		else
...
static void sun4i_usb_phy0_id_vbus_det_scan(struct work_struct *work)
{
	...
	dev_dbg(&phy0->dev, "ID Det: %d; VBus Det: %d; dr_mode: %d\n",
			id_det, vbus_det, data->dr_mode);
...
static void sun4i_usbc_update_cb(void* arg) {
	struct sun4i_usb_phy_data *data = (struct sun4i_usb_phy_data *)arg;

	/* usb c state updated- refresh the id pin */
	mod_delayed_work(system_wq, &data->detect, DEBOUNCE_TIME);
}
...
static int sun4i_usb_phy_probe(struct platform_device *pdev)
{
	...
	if (of_property_present(np, "axp2585-bmu")) {
		int err;
		data->axp2585_pdev = axp2585_batt_by_phandle(&pdev->dev, 0);
		if (IS_ERR(data->axp2585_pdev)) {
			err = PTR_ERR(data->axp2585_pdev);
			dev_dbg(&pdev->dev, "Failed to load axp2585 device: %d\n", err);
			return err;
		}
	}
	else {
		data->axp2585_pdev = NULL;
	}
	...
	if (data->axp2585_pdev) {
		axp2585_batt_register_det_cb(data->axp2585_pdev, sun4i_usbc_update_cb, data);
	}
