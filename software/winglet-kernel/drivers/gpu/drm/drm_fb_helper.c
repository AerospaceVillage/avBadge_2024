...
static int drm_fb_helper_single_fb_probe(struct drm_fb_helper *fb_helper)
{
	...
	fb_helper->screen_width = sizes.fb_width;
	fb_helper->screen_height = sizes.fb_height;

	if (fb_helper->surface_override_en) {
		/* Only set output screen width/height with params if not scaling */
		if (!(fb_helper->surface_override.fb_flags & DRM_MODE_FB_ALLOW_SCALING)) {
			fb_helper->screen_width = fb_helper->surface_override.fb_width;
			fb_helper->screen_height = fb_helper->surface_override.fb_height;
		}
		fb_helper->surface_override.surface_bpp = sizes.surface_bpp;
		fb_helper->surface_override.surface_depth = sizes.surface_depth;

		ret = (*fb_helper->funcs->fb_probe)(fb_helper, &fb_helper->surface_override);
	} else {
...
int drm_fb_helper_hotplug_event(struct drm_fb_helper *fb_helper)
{
	...
	drm_client_modeset_probe(&fb_helper->client, fb_helper->screen_width,
				 fb_helper->screen_height);
	drm_setup_crtcs_fb(fb_helper);
	mutex_unlock(&fb_helper->lock);
