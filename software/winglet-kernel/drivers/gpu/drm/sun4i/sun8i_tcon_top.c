...
int sun8i_tcon_top_de_config(struct device *dev, int mixer, int tcon)
{
	...
	if (quirks->dont_touch_port_sel) {
		/* Some chips touching port sel glitches the screen green */
		return 0;
	}
...
static const struct sun8i_tcon_top_quirks sun20i_d1_tcon_top_quirks = {
	.has_dsi	= true,
	.dont_touch_port_sel	=	true,  /* Don't touch port sel regs. They
										  aren't documented and break stuff */
};
