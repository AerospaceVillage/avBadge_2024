...
static int sun6i_rtc_probe(struct platform_device *pdev)
{
	...
	/* Force external oscillator if requested */
	if (of_property_read_bool(pdev->dev.of_node, "force-ext-osc"))
	{
		u32 val = SUN6I_LOSC_CTRL_KEY | SUN6I_LOSC_CTRL_EXT_LOSC_EN |
			  SUN6I_LOSC_CTRL_EXT_OSC;
		writel(val, chip->base + SUN6I_LOSC_CTRL);
	}
