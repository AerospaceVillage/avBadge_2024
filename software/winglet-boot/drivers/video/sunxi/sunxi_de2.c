...
static void sunxi_de2_composer_init(void)
{
	...
	/* Start with pll3 @ 300 MHz (it works)
	 * When the LCD reconfigures it'll adjust this, but that's okay since we just need to feed the LCD fast enough
	 * This config appears to do so */

	clock_set_pll3(300000000);

	/* Set /4 from parent clock */
	clrsetbits_le32(&ccm->de_clk_cfg, CCM_DE2_CTRL_PLL_MASK,
			CCM_DE2_CTRL_M(4));

	/* Set DE parent to pll3 (video0 4x) */
	clrsetbits_le32(&ccm->de_clk_cfg, CCM_DE2_CTRL_PLL_MASK,
			CCM_DE2_CTRL_PLL3_4X);

	/* Set ahb gating to pass */
	setbits_le32(&ccm->de_gate_reset, 1 << RESET_SHIFT);
	setbits_le32(&ccm->de_gate_reset, 1 << GATE_SHIFT);
