			 ...
			 int depth, int dclk_phase)
{
	...
	if (mode->flags & DISPLAY_FLAGS_PIXDATA_POSEDGE)
		val |= SUNXI_LCDC_TCON0_IO_POL_DCLK_INV;
		  ...
		  int *clk_div, int *clk_double, bool is_composite)
{
	...
	clock_set_pll3_factors(best_n, best_double);

	if (tcon == 0) {
		/* Set /1 from parent clock */
		clrsetbits_le32(&ccm->tcon_lcd0_clk_cfg, CCM_TCONLCD_CTRL_PLL_MASK,
				CCM_TCONLCD_CTRL_M(1));

		/* Set DE parent to pll3/4 (video0 1x) */
		clrsetbits_le32(&ccm->tcon_lcd0_clk_cfg, CCM_TCONLCD_CTRL_PLL_MASK,
				CCM_TCONLCD_CTRL_PLL3_4X);

		/* Clock on */
		setbits_le32(&ccm->tcon_lcd0_clk_cfg, CCM_TCONLCD_CTRL_GATE);
	}
