...
SPLASH_DECL(fake_pwroff_logo);

void *video_get_fake_pwroff_logo(void)
{
	return SPLASH_START(fake_pwroff_logo);
}

SPLASH_DECL(batt_low_logo);

void *video_get_batt_low_logo(void)
{
	return SPLASH_START(batt_low_logo);
}

static int video_splash_disabled = 0;
void video_disable_splash(int disable) {
	video_splash_disabled = disable;
}
#endif
...
static int show_splash(struct udevice *dev)
{
	...
	ret = video_bmp_display(dev, map_to_sysmem(data), CONFIG_VIDEO_LOGO_X, CONFIG_VIDEO_LOGO_Y, true);
