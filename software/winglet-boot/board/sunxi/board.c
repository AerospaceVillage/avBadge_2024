...
#ifdef CONFIG_I2C_PROBE_ON_INIT
#include <i2c.h>
...
#if defined(CONFIG_I2C_PROBE_ON_INIT) && !defined(CONFIG_SPL_BUILD)
#ifndef CONFIG_I2C_DEFAULT_BUS_NUMBER
#error You must specify a default bus number
#else
	int ret;
...
void sunxi_board_init(void)
{
	...
	defined CONFIG_AXP313_POWER || defined AXP2585_POWER
...
#ifdef CONFIG_SUNXI_USES_WINGLET_SERIAL
/* add early init board specific code for spl here */
void spl_board_init(void)
{
	// Enable JTAG
	uint32_t data = readl(0x020000c4) & 0xFFFF0000;
	data |= 0x6666;
	writel(data, 0x020000c4);

	// Extract serial number
	u32 serial_entry = (u32)readl(SUNXI_SID_BASE + SUNXI_SID_OEMINFO_OFFSET);
	u16 serial_norm = serial_entry & 0xFFFF;
	u16 serial_inv = (serial_entry >> 16) & 0xFFFF;
	if (serial_norm != (serial_inv ^ 0xFFFF)) {
		panic("Serial number invalid: Cannot determine model!");
	}

	// Extract model string from device tree
	const size_t max_model_len = 4;
	char model_str[max_model_len + 1];
	int i = 0;

	const char *dt_str = get_spl_dt_name() + CONFIG_SUNXI_WINGLET_MODEL_NAME_OFFSET;
	if (*dt_str++ != '-') {
		panic("Invalid Compiled Device Tree Name");
	}
	while (*dt_str != 0 && *dt_str != '-' && i < max_model_len) {
		model_str[i++] = *dt_str++;
	}
	if (*dt_str != '-') {
		panic("Invalid Compiled Device Tree Name");
	...
	// Determine model info from model string
	const char* model_name = "";

	// Print info
	u16 model_val = (serial_norm >> 12) & 0xF;
	if (model_val == 1) {
		model_name = "Bring-up-Board";
	}
	else if (model_val == 2) {
		model_name = "Devkit";
	}
	...
	u16 serial_num = serial_norm & 0xFFF;
	printf("Model: Winglet %s (Serial: %d)\n", model_name, serial_num);

	if (!strcmp(model_str, "dk")) {
		if (model_val != 3 && model_val != 2) {
			panic("Unexpected Board Model, image for Prod/Devkit");
		}
	}
	else if (!strcmp(model_str, "bub")) {
		if (model_val != 1) {
			panic("Unexpected Board Mode, image for Bring-up-Board");
		}
	}
	else {
		panic("Unexpected Board Model Variant");
	}
}
#endif
...
#ifdef CONFIG_SUNXI_USES_WINGLET_SERIAL
#error Serial tag not supported with winglet serial
#endif
...
#ifdef CONFIG_SUNXI_USES_WINGLET_SERIAL
	if (!env_get("serial#")) {
		u16 serial_entry = (u16)readl(SUNXI_SID_BASE + SUNXI_SID_OEMINFO_OFFSET);
		u16 model_nibble = (serial_entry >> 12) & 0xF;
		u16 serial_num = serial_entry & 0xFFF;
		if (model_nibble != 0) {
			char model_char = 64 + model_nibble;
			snprintf(serial_string, sizeof(serial_string), "W%c%04d",
				model_char, serial_num);
...
static void serial_number_dt_fixup(void* blob) {
	const char* serial_num = env_get("serial#");
	if (serial_num) {
		// Add serial number if it exists
		do_fixup_by_path(blob, "/", "serial-number", serial_num,
				strlen(serial_num), 1);
	}

	unsigned int sid[4];
	if (!sunxi_get_sid(sid)) {
		// Add SID to the device tree
		do_fixup_by_path(blob, "/", "sid", sid, sizeof(sid), 1);
	}

	uint64_t signature = 0xCECAA2AAC2CAAC84;
	do_fixup_by_path(blob, "/", "signature", &signature, sizeof(signature), 1);
}
